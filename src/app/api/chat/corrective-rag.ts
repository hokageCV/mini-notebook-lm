...
export async function corrective_rag(user_query: string) {
  ...
  let retry_good_results = retry_results.filter(([doc, score]) => score >= MIN_SCORE)
  if (retry_good_results.length > 0) {
    return retry_good_results.map(([doc]) => doc)
  }

  let hyde_doc = await client.chat.completions.create({
    model: 'gpt-5-nano',
    messages: [
      { role: 'system', content: 'Generate a hypothetical best-guess answer to this query, based on general knowledge.' },
      { role: 'user', content: user_query }
    ]
  })
  let synthetic_doc = hyde_doc.choices[0]?.message?.content ?? ''
  console.log('DBG:',synthetic_doc);
  let hyde_results = await vector_store.similaritySearchWithScore(synthetic_doc, 3)
  relevant_chunk = hyde_results.map(([doc]) => doc)
